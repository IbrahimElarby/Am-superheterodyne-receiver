...
clf;
Trange = [0 5 -2 2];
Frange1 = [-200*10^3 200*10^3 0 8000];
Frange2 = [-200*10^3 200*10^3 0 8000];
%% Message
...
B=6*10^3;
[music1, ~] = audioread('sample1.wav');
[music2, ~] = audioread('sample2.wav');
[music3, fs1] = audioread('sample3.wav');
...
DSB1 = fftshift(fft(dsb1,Lfft));
DSB2 = fftshift(fft(dsb2,Lfft));
DSB3 = fftshift(fft(dsb3,Lfft));
...
DEM1 = fftshift(fft(dem1, Lfft));
DEM2 = fftshift(fft(dem2, Lfft));
DEM3 = fftshift(fft(dem3, Lfft));
...
RFAMP_s=fftshift(fft(rfamp, Lfft));
subplot(5,2,4);
...
IFF = fftshift(fft(iff, Lfft)); %Freq Response of Filtered Stations
...
RECDEM = fftshift(fft(recdem, Lfft));
